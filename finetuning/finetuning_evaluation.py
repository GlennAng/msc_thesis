...
def compute_test_embeddings(finetuning_model : FinetuningModel, test_papers : dict, embeddings_folder : str) -> None:
    ...
    category_idx_l2_tensor = test_papers["category_idx_l2"] if "category_idx_l2" in test_papers else None
    embeddings = finetuning_model.compute_papers_embeddings(test_papers["input_ids"], test_papers["attention_mask"], test_papers["category_idx"], category_idx_l2_tensor)
...
def run_validation(finetuning_model : FinetuningModel, val_ratings : FinetuningDataset, val_negative_samples : dict, print_results : bool = True) -> tuple:
    ...
    training_mode = finetuning_model.training
    finetuning_model.eval()
    val_users_scores = finetuning_model.compute_val_ratings_scores(val_ratings.input_ids_tensor, val_ratings.attention_mask_tensor, val_ratings.category_idx_tensor, 
                                                                   val_ratings.user_idx_tensor, val_ratings.category_idx_l2_tensor)
    val_negative_samples_scores = finetuning_model.compute_val_negative_samples_scores(val_negative_samples["input_ids"], val_negative_samples["attention_mask"], 
                                                                                       val_negative_samples["category_idx"], val_negative_samples.get("category_idx_l2", None))
...
if __name__ == "__main__":
    ...
    if evaluation_config["include_l2_categories"]:
    ...
    papers_ids_to_categories_idxs_l2 = load_papers_ids_to_categories_idxs_l2() if evaluation_config["include_l2_categories"] else None
    test_papers = load_papers(path = "test", papers_ids_to_categories_idxs = papers_ids_to_categories_idxs, papers_ids_to_categories_idxs_l2 = papers_ids_to_categories_idxs_l2)
    finetuning_model = load_finetuning_model(evaluation_config["state_dicts_folder"], device, n_unfreeze_layers = 0, n_categories_l2 = n_categories_l2)
