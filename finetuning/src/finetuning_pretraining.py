...
from create_example_configs import create_example_config
...
def create_finetuning_configs(train_users: list, val_users: list) -> None:
    ...
    command = f"python {ProjectPaths.logreg_path() / 'run.py'} --config {finetuning_config_path}"
    os.system(command)
...
def load_outputs() -> tuple:
    ...
    assert os.path.exists(finetuning_outputs_path_train) and os.path.exists(finetuning_outputs_path_val)
    ...
    assert list(train_users_ids_to_idxs.keys()) == train_users and list(val_users_ids_to_idxs.keys()) == val_users
    return train_coefs, val_coefs, train_users_ids_to_idxs, val_users_ids_to_idxs

def save_outputs(train_coefs: np.ndarray, val_coefs: np.ndarray, train_users_ids_to_idxs: dict, val_users_ids_to_idxs: dict) -> None:
    coefs_merged = np.concatenate((train_coefs, val_coefs), axis=0)
    assert coefs_merged.shape[0] == len(train_users) + len(val_users)
    val_users_ids_to_idxs_updated = {user_id: idx + len(train_users) for user_id, idx in val_users_ids_to_idxs.items()}
    ...
    np.save(ProjectPaths.finetuning_data_model_state_dicts_path() / "users_coefs.npy", coefs_merged)
    print(f"Saved users_coefs.npy with shape {coefs_merged.shape} at {ProjectPaths.finetuning_data_model_state_dicts_path() / 'users_coefs.npy'}")
    with open(ProjectPaths.finetuning_data_model_state_dicts_path() / "users_coefs_ids_to_idxs.pkl", "wb") as f:
        pickle.dump(users_ids_to_idxs, f)
    print(f"Saved users_coefs_ids_to_idxs.pkl with {len(users_ids_to_idxs)} entries at {ProjectPaths.finetuning_data_model_state_dicts_path() / 'users_coefs_ids_to_idxs.pkl'}")
