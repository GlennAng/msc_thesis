...
class FinetuningModel(nn.Module):
    ...
    def forward(self, eval_type: str, input_ids_tensor: torch.Tensor, attention_mask_tensor: torch.Tensor, category_l1_tensor: torch.Tensor,
                category_l2_tensor: torch.Tensor = None, user_idx_tensor: torch.Tensor = None,
        ...
        t = self.check_devices(input_ids_tensor, attention_mask_tensor, category_l1_tensor, category_l2_tensor, user_idx_tensor, 
                               sorted_unique_user_idx_tensor, batch_negatives_indices)
        input_ids_tensor, attention_mask_tensor, category_l1_tensor, category_l2_tensor, user_idx_tensor, sorted_unique_user_idx_tensor, batch_negatives_indices = t
    ...
    def check_devices(self, input_ids_tensor: torch.Tensor, attention_mask_tensor: torch.Tensor, category_l1_tensor: torch.Tensor,
                            category_l2_tensor: torch.Tensor = None, user_idx_tensor: torch.Tensor = None, sorted_unique_user_idx_tensor: torch.Tensor = None, 
        ...
        return (input_ids_tensor, attention_mask_tensor, category_l1_tensor, category_l2_tensor, user_idx_tensor,
                sorted_unique_user_idx_tensor, batch_negatives_indices)
    ...
    def save_finetuning_model(self, model_path: Path) -> None:
        ...
        torch.save(self.categories_embeddings_l1.state_dict(), model_path / "categories_embeddings_l1.pt")
        if self.categories_embeddings_l2 is not None:
    ...
    def perform_inference(self, max_batch_size: int, input_ids_tensor: torch.Tensor, attention_mask_tensor: torch.Tensor, category_l1_tensor: torch.Tensor,
                          category_l2_tensor : torch.Tensor = None, user_idx_tensor : torch.Tensor = None, eval_type : str = None) -> torch.Tensor:
        MAX_TRIES = 10
        results_total = []
        ...
        while n_samples_processed < n_samples:
            ...
            while n_tries_so_far < MAX_TRIES:
                try:
                    ...
                    if category_l2_tensor is not None:
                        batch_category_l2_tensor = category_l2_tensor[n_samples_processed : upper_bound]
                    ...
                    with torch.autocast(device_type = self.device.type, dtype = torch.float16):
                        with torch.inference_mode():
                            results = self(eval_type = eval_type, input_ids_tensor = batch_input_ids_tensor, attention_mask_tensor = batch_attention_mask_tensor,
                                           category_l1_tensor = batch_category_l1_tensor, category_l2_tensor = batch_category_l2_tensor,
    ...
    def compute_papers_embeddings(self, input_ids_tensor: torch.Tensor, attention_mask_tensor: torch.Tensor, category_l1_tensor: torch.Tensor,
                                category_l2_tensor : torch.Tensor = None) -> torch.Tensor:
        return self.perform_inference(self.eval_batch_size, input_ids_tensor, attention_mask_tensor, category_l1_tensor,
                                category_l2_tensor = category_l2_tensor, eval_type = "test")

    def compute_val_dataset_scores(self, input_ids_tensor: torch.Tensor, attention_mask_tensor: torch.Tensor, category_l1_tensor: torch.Tensor, 
                                category_l2_tensor: torch.Tensor = None, user_idx_tensor: torch.Tensor = None) -> torch.Tensor:
        assert user_idx_tensor is not None
        return self.perform_inference(self.eval_batch_size, input_ids_tensor, attention_mask_tensor, category_l1_tensor,
                                category_l2_tensor = category_l2_tensor, user_idx_tensor = user_idx_tensor, 
                                eval_type = "val")

    def compute_val_negative_samples_scores(self, input_ids_tensor: torch.Tensor, attention_mask_tensor: torch.Tensor, category_l1_tensor: torch.Tensor,
                                            category_l2_tensor: torch.Tensor = None) -> torch.Tensor:
        with torch.autocast(device_type = self.device.type, dtype = torch.float16):
            with torch.inference_mode():
                return self(eval_type = "negative_samples", input_ids_tensor = input_ids_tensor, attention_mask_tensor = attention_mask_tensor,
                            category_l1_tensor = category_l1_tensor, category_l2_tensor = category_l2_tensor, 
                            sorted_unique_user_idx_tensor = self.val_users_embeddings_idxs).cpu()
...
def load_projection(device: torch.device, transformer_embeddings_dim: int = None, projection_dim: int = None, projection_state_dict: dict = None, 
...
def init_categories_embeddings_l2(device: torch.device, num_embeddings: int, embeddings_dim: int, dtype = torch.float32) -> nn.Embedding:
...
def load_finetuning_model(finetuning_model_path: Path, device: torch.device, mode: str, n_unfreeze_layers: int = 4, val_users_embeddings_idxs: torch.Tensor = None,
                          projection_state_dict: dict = None, users_embeddings_state_dict: dict = None, 
                          categories_embeddings_l1_state_dict: dict = None, categories_embeddings_l2_state_dict: dict = None,
                          pretrained_projection: bool = True, pretrained_users_embeddings: bool = True, pretrained_categories_embeddings_l1: bool = True,
                          projection_dim: int = None, n_users: int = None, n_categories_l1 : int = None, n_categories_l2: int = None,
                          categories_embeddings_dim: int = None) -> FinetuningModel:
    modes = ["train", "eval"]
    ...
    if mode == "eval" or (mode == "train" and projection_state_dict is None and pretrained_projection):
        ...
        projection = load_projection(device, transformer_embeddings_dim = transformer_embeddings_dim, projection_dim = projection_dim)

    if mode == "eval" or (mode == "train" and categories_embeddings_l1_state_dict is None and pretrained_categories_embeddings_l1):
        categories_embeddings_l1_state_dict = torch.load(finetuning_model_path / "categories_embeddings_l1.pt", map_location = device, weights_only = True)
        categories_embeddings_l1 = load_embeddings(device, embeddings_state_dict = categories_embeddings_l1_state_dict)
    else:
        categories_embeddings_l1 = load_embeddings(device, num_embeddings = n_categories_l1, embeddings_dim = categories_embeddings_dim)
    categories_embeddings_l1_dim = categories_embeddings_l1.embedding_dim

    if mode == "eval" or (mode == "train" and users_embeddings_state_dict is None and pretrained_users_embeddings):
        users_embeddings_state_dict = torch.load(finetuning_model_path / "users_embeddings.pt", map_location = device, weights_only = True)
        users_embeddings = load_embeddings(device, embeddings_state_dict = users_embeddings_state_dict)
    else:
        users_embeddings = load_embeddings(device, num_embeddings = n_users, embeddings_dim = transformer_embeddings_dim + categories_embeddings_l1_dim + 1)

    categories_embeddings_l2 = None
    if mode == "eval":
        categories_embeddings_l2_path = finetuning_model_path / "categories_embeddings_l2.pt"
        if os.path.exists(categories_embeddings_l2_path) and categories_embeddings_l2_state_dict is None:
            categories_embeddings_l2_state_dict = torch.load(categories_embeddings_l2_path, map_location = device, weights_only = True)
        if categories_embeddings_l2_state_dict is not None:
            categories_embeddings_l2 = load_embeddings(device, embeddings_state_dict = categories_embeddings_l2_state_dict)
    if mode == "train" and n_categories_l2 is not None:
        categories_embeddings_l2 = init_categories_embeddings_l2(device, num_embeddings = n_categories_l2, embeddings_dim = categories_embeddings_l1_dim)

    return FinetuningModel(transformer_model, projection, users_embeddings, categories_embeddings_l1, categories_embeddings_l2,
                           val_users_embeddings_idxs = val_users_embeddings_idxs, n_unfreeze_layers = n_unfreeze_layers)