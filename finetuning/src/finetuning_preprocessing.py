...
from load_files import load_finetuning_users, load_papers, load_papers_texts, load_users_ratings
from papers_categories import get_glove_categories_embeddings
from papers_categories_dicts import PAPERS_CATEGORIES
from training_data import get_filtered_cache_papers_ids_for_user, get_negative_samples_ids, load_negrated_ranking_ids_for_user

def load_users_coefs_ids_to_idxs() -> dict:
    users_coefs_ids_to_idxs_path = ProjectPaths.finetuning_data_model_state_dicts_path() / "users_coefs_ids_to_idxs.pkl"
    if not users_coefs_ids_to_idxs_path.exists():
        raise FileNotFoundError(f"Users coefs IDs to indices mapping file not found: {users_coefs_ids_to_idxs_path}.")
...
def load_val_users_embeddings_idxs(val_users_ids: list = None, users_coefs_ids_to_idxs: dict = None) -> torch.Tensor:
    ...
    assert len(val_users_embeddings_idxs) == len(set(val_users_embeddings_idxs)) and val_users_embeddings_idxs == sorted(val_users_embeddings_idxs)
    return torch.tensor(val_users_embeddings_idxs, dtype = torch.int64)

def save_users_embeddings_tensor(users_coefs: np.ndarray = None, users_coefs_ids_to_idxs: dict = None) -> None:
    ...
    if users_embeddings_path.exists():
        print(f"Users embeddings tensor already exists - skipping saving.")
        return
    if users_coefs is None:
        users_coefs = np.load(ProjectPaths.finetuning_data_model_state_dicts_path() / "users_coefs.npy")
    if users_coefs_ids_to_idxs is None:
        users_coefs_ids_to_idxs = load_users_coefs_ids_to_idxs()
...
def save_projection_tensor(pca_components: np.ndarray = None, pca_mean: np.ndarray = None) -> None:
    ...
    if projection_tensor_path.exists():
        print(f"Projection tensor already exists - skipping saving.")
        return
    model_name = ProjectPaths.finetuning_data_model_path().stem
    pca_path = ProjectPaths.logreg_embeddings_path() / "after_pca" / model_name
    pca_components = np.load(pca_path / "pca_components.npy") if pca_components is None else pca_components
...
def save_categories_embeddings_tensor(glove_categories_embeddings: dict = None) -> None:
    categories_embeddings_l1_path = ProjectPaths.finetuning_data_model_state_dicts_categories_embeddings_l1_path()
    categories_to_idxs_l1_path = ProjectPaths.finetuning_data_model_state_dicts_path() / "categories_to_idxs_l1.pkl"
    categories_to_idxs_l2_path = ProjectPaths.finetuning_data_model_state_dicts_path() / "categories_to_idxs_l2.pkl"
    paths_existing = [categories_embeddings_l1_path.exists(), categories_to_idxs_l1_path.exists(), categories_to_idxs_l2_path.exists()]
    if all(paths_existing):
        print(f"Categories embeddings tensors and indices dicts already exist - skipping saving.")
        return
    if any(paths_existing):
        raise FileExistsError(f"One or more but not all of the files already exist:" +
                              f"{categories_embeddings_l1_path}, {categories_to_idxs_l1_path}, {categories_to_idxs_l2_path}.")
    papers = load_papers()
    ...
    if glove_categories_embeddings is None:
        glove_categories_embeddings = get_glove_categories_embeddings(PAPERS_CATEGORIES.categories_to_glove)
    assert isinstance(glove_categories_embeddings, dict) and set(glove_categories_embeddings.keys()) == set(categories_l1)
    ...
    categories_embeddings_l1 = torch.nn.Embedding(len(categories_l1), len(glove_categories_embeddings[categories_l1[0]]), dtype = torch.float32)
    for idx, category in enumerate(categories_l1):
        with torch.no_grad():
            categories_embeddings_l1.weight.data[idx] = torch.from_numpy(glove_categories_embeddings[category]).to(torch.float32)
...
def finetuning_tokenize_papers(papers_ids: list, tokenizer: AutoTokenizer, max_sequence_length: int) -> tuple:
    ...
    papers_texts = load_papers_texts(relevant_papers_ids = papers_ids, relevant_columns = ["paper_id", "title", "abstract"])
...
def save_val_negative_samples(tokenizer: AutoTokenizer, max_sequence_length: int, n_negative_samples: int = 100, random_state: int = VAL_RANDOM_STATE) -> None:
    tensor_path = ProjectPaths.finetuning_data_model_datasets_val_negative_samples_path()
    if tensor_path.exists():
        print(f"Validation negative samples already exist - skipping saving.")
        return
    papers = load_papers(relevant_columns = ["paper_id", "in_ratings", "in_cache", "l1"])
    negative_samples_ids = get_negative_samples_ids(papers, n_negative_samples, random_state, exclude_in_ratings = True, exclude_in_cache = True)
    negative_samples_ids = sorted(list(negative_samples_ids))
    negative_samples = finetuning_tokenize_papers(negative_samples_ids, tokenizer, max_sequence_length)[0]
    torch.save(negative_samples, tensor_path)
    print(f"Validation negative samples tensor saved to {tensor_path}.")

def finetuning_get_cache_papers_ids_for_user(users_ratings: pd.DataFrame, user_id: int, cache_ids: list, max_cache: int, random_state: int) -> set:
    ...
    user_cache_ids = get_filtered_cache_papers_ids_for_user(cache_ids, rated_ids, max_cache, random_state)
    return set(user_cache_ids)

def save_test_papers(tokenizer: AutoTokenizer, max_sequence_length: int, n_cache: int = 5000, n_cache_attached: int = 5000, n_negative_samples: int = 100, 
                     val_random_state: int = VAL_RANDOM_STATE, test_random_states: list = TEST_RANDOM_STATES) -> None:
    ...
    if tensor_path.exists():
        print(f"Test papers already exist - skipping saving.")
        return
    papers = load_papers(relevant_columns = ["paper_id", "in_ratings", "in_cache", "l1"])
    cache_ids = papers[papers["in_cache"] == True]["paper_id"].tolist()
    ...
    for random_state in [val_random_state] + test_random_states:
        negative_samples_ids = get_negative_samples_ids(papers, n_negative_samples, random_state, exclude_in_ratings = True, exclude_in_cache = True)
        cache_attached_papers_ids = get_negative_samples_ids(papers, n_cache_attached, random_state, papers_to_exclude = negative_samples_ids)
        papers_ids.update(negative_samples_ids + cache_attached_papers_ids)
    val_users_ids = load_finetuning_users(selection = "val")
    test_users_ids = load_finetuning_users(selection = "test")
    users_ratings = load_users_ratings(relevant_columns = ["user_id", "paper_id", "rating"], relevant_users_ids = val_users_ids + test_users_ids)
    papers_ids.update(users_ratings["paper_id"].unique())
    for val_user_id in tqdm(val_users_ids, desc = "Getting Cache Papers for Val Users", unit = "Val User"):
        papers_ids.update(finetuning_get_cache_papers_ids_for_user(users_ratings, val_user_id, cache_ids, n_cache, val_random_state))
    for test_user_id in tqdm(test_users_ids, desc = "Getting Cache Papers for Test Users", unit = "Test User"):
        for random_state in test_random_states:
            papers_ids.update(finetuning_get_cache_papers_ids_for_user(users_ratings, test_user_id, cache_ids, n_cache, random_state))
...
def save_train_val_users_papers(papers_type: str, tokenizer: AutoTokenizer, max_sequence_length: int) -> None:
    ...
    papers_ids_to_idxs_path = ProjectPaths.finetuning_data_model_datasets_path() / f"{papers_type}_papers_ids_to_idxs.pkl"
    ...
    if tensor_path.exists() or papers_ids_to_idxs_path.exists():
        raise FileExistsError(f"One or more but not all of the files already exist: {tensor_path}, {papers_ids_to_idxs_path}.")
    selection = "train" if papers_type == "train_users" else "val"
    users_ids = load_finetuning_users(selection = selection)
    users_ratings = load_users_ratings(relevant_columns = ["user_id", "paper_id"], relevant_users_ids = users_ids)
    papers_ids = sorted(list(users_ratings["paper_id"].unique()))
    papers_dict, papers_ids_to_idxs = finetuning_tokenize_papers(papers_ids, tokenizer, max_sequence_length)
    ...
    print(f"{papers_type.capitalize()} papers IDs to indices mapping saved to {papers_ids_to_idxs_path}.")
...
def save_finetuning_papers_type(papers_type: str, tokenizer: AutoTokenizer = None, max_sequence_length: int = 512) -> None:
    ...
    if papers_type == "val_negative_samples":
        ...
        raise ValueError(f"Invalid papers type: {papers_type}. Choose from ['val_negative_samples', 'test', 'train_users', 'val_users'].")
...
def load_finetuning_papers(papers_type: str, attach_l1: bool = True, attach_l2: bool = True) -> dict:
    papers_types = ["val_negative_samples", "test", "train_users", "val_users"]
    if papers_type not in papers_types:
        raise ValueError(f"Invalid papers type: {papers_type}. Choose from {papers_types}.")
    if papers_type == "val_negative_samples":
    ...
    if attach_l1 or attach_l2:
        papers = load_papers(relevant_columns = ["paper_id", "l1", "l2"], relevant_papers_ids = papers_tensor["paper_id"].tolist())
        ...
        if attach_l1:
            ...
            categories_l1 = [category if category in categories_to_idxs_l1 else None for category in categories_l1]
            papers_tensor["l1"] = torch.tensor([categories_to_idxs_l1[category] for category in categories_l1])
        if attach_l2:
            categories_to_idxs_l2 = load_categories_to_idxs("l2")
            categories_l2 = papers["l2"].tolist()
            categories_l2 = [category if category in categories_to_idxs_l2 else None for category in categories_l2]
            papers_tensor["l2"] = torch.tensor([categories_to_idxs_l2[category] for category in categories_l2])
...
def setup_finetuning_papers_dataset(dataset_type: str, users_type: str) -> tuple:
    ...
    if dataset_type == "train" and users_type not in ["train_users", "val_users"]:
        raise ValueError(f"Invalid users type: {users_type}. Choose from ['train_users', 'val_users'] for train dataset.")
    if dataset_type == "val" and users_type != "val_users":
        raise ValueError(f"Invalid users type: {users_type}. Choose 'val_users' for validation dataset.")
    users_ids = load_finetuning_users(selection = "train") if users_type == "train_users" else load_finetuning_users(selection = "val")
    ...
    if users_type == "train_users":
        users_ratings = load_users_ratings(relevant_columns = ["user_id", "paper_id", "rating", "session_id"], relevant_users_ids = users_ids)
    else:
        users_ratings = get_users_ratings(users_selection = users_ids, evaluation = Evaluation.SESSION_BASED, train_size = 1.0,
                                          min_n_posrated_train = 16, min_n_negrated_train = 16, min_n_posrated_val = 4, min_n_negrated_val = 4)[0]
...
def fill_finetuning_papers_dataset(users_ratings: pd.DataFrame, papers: pd.DataFrame, papers_ids_to_idxs: dict, users_coefs_ids_to_idxs: dict,
                                   negrated_ranking: bool) -> dict:
    unique_users_ids = users_ratings["user_id"].unique().tolist()
    assert len(unique_users_ids) == len(set(unique_users_ids)) and unique_users_ids == sorted(unique_users_ids)
    user_idx_list, paper_id_list, rating_list, input_ids_list, attention_mask_list = [], [], [], [], []
    category_l1_list, category_l2_list, session_id_list = [], [], []
    for user_id in unique_users_ids:
        user_ratings = users_ratings[users_ratings["user_id"] == user_id]
        n_ratings = len(user_ratings)
        assert user_ratings["session_id"].is_monotonic_increasing
        user_idx_list.extend([users_coefs_ids_to_idxs[user_id]] * n_ratings)
        assert user_ratings["paper_id"].nunique() == n_ratings
        paper_to_session_id = dict(zip(user_ratings["paper_id"], user_ratings["session_id"]))
        ...
        for paper_id in rated_ids:
            ...
            session_id_list.append(paper_to_session_id[paper_id])
    ...
    assert len(users_ratings) == len(user_idx_list) == len(paper_id_list) == len(input_ids_list) == len(attention_mask_list)
    assert len(users_ratings) == len(category_l1_list) == len(category_l2_list) == len(session_id_list)
    return {
        "user_idx": user_idx_list, "paper_id": paper_id_list, "rating": rating_list,
        "input_ids": input_ids_list, "attention_mask": attention_mask_list,
        "category_l1": category_l1_list, "category_l2": category_l2_list, "session_id": session_id_list
    }

def get_finetuning_papers_dataset(dataset_type: str, users_type: str, users_coefs_ids_to_idxs: dict) -> dict:
    users_ratings, papers, papers_ids_to_idxs = setup_finetuning_papers_dataset(dataset_type, users_type)
    dataset = fill_finetuning_papers_dataset(users_ratings, papers, papers_ids_to_idxs, users_coefs_ids_to_idxs, dataset_type == "val")
    ...
    if users_type == "train_users":
        ...
        assert sorted(list(user_idx_set)) == list(range(user_idx_min, user_idx_min + len(user_idx_set)))
    return dataset
...
def save_finetuning_dataset(dataset_type: str) -> None:
    ...
    users_coefs_ids_to_idxs = load_users_coefs_ids_to_idxs()
    if dataset_type == "train":
        train_papers_train_users = get_finetuning_papers_dataset("train", "train_users", users_coefs_ids_to_idxs)
        train_papers_val_users = get_finetuning_papers_dataset("train", "val_users", users_coefs_ids_to_idxs)
        dataset = {key: train_papers_train_users[key] + train_papers_val_users[key] for key in train_papers_train_users}
    elif dataset_type == "val":
    ...
    dataset = {
        ...
        "session_id": torch.tensor(dataset["session_id"], dtype = torch.int64)
    }
