...
from ..embeddings.embedding import Embedding, compute_cosine_similarities
...
def train_wordclouds(
    ...
    assert len(pos_train_ids) == len(pos_train_cluster_labels)
    ...
    for cluster_idx in range(n_clusters):
        cluster_pos_train_ids = [
            paper_id
            for paper_id, cluster_label in zip(pos_train_ids, pos_train_cluster_labels)
            if cluster_label == cluster_idx
        ]
        cluster_mean_embedding = get_mean_embedding(v, cluster_pos_train_ids)
        cluster_scores = {word: score for word, score in zip(features, cluster_mean_embedding.A1)}
        clusters_dict[cluster_idx] = {
            "words_scores": cluster_scores,
            "n_pos_train_papers": len(cluster_pos_train_ids),
        }
    return pos_train_scores, neg_train_scores, clusters_dict
...
class User_Visualizer:
    ...
    def visualize_wordclouds(
        ...
        wc_pos_train_scores, wc_neg_train_scores, clusters_dict = train_wordclouds(
            ...
            pos_train_cluster_labels,
        )
        wc_words_scores = merge_tfidf_scores(wc_pos_train_scores, wc_neg_train_scores)
        n_pos_train_papers_full = len(
            [label for label in fold_predictions["train_labels"] if label == 1]
        )
        n_neg_train_papers_full = len(
            [label for label in fold_predictions["train_labels"] if label == 0]
        )
        generate_wordclouds(
            pdf=pdf,
    ...
    def visualize_papers(
        ...
        pdf: PdfPages,
        fold_idx: int,
        wc_words_scores: dict,
        papers: pd.DataFrame,
        train_cluster_labels: np.ndarray,
        val_cluster_labels: np.ndarray,
    ) -> None:
        ...
        fold_train_predictions_df["cluster_label"] = train_cluster_labels
        ...
        fold_val_predictions_df["cluster_label"] = val_cluster_labels
...
if __name__ == "__main__":
    ...
    eval_settings = args["outputs_folder"].parent / "eval_settings.json"
    with open(eval_settings, "r") as f:
        eval_settings = json.load(f)
    n_clusters = eval_settings.get("clustering_k_means_n_clusters", 1)
    ...
    embedding = Embedding(config["embedding_folder"])
    ...
    for user_id in args["users_ids"]:
        ...
        assert np.array_equal(
            pos_train_cluster_labels, train_cluster_labels[train_ratings["rating"] == 1]
        )
        val_cluster_labels = kmeans.predict(
            embedding.matrix[embedding.get_idxs(val_ratings["paper_id"].tolist())]
        )
