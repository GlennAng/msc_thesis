...
HYPERPARAMETERS_ABBREVIATIONS = {
    "clf_C": "C",
    "weights_cache_v": "v",
    "weights_negrated_importance": "v",
    "weights_cache_v1_v2": "v1_v2",
    "weights_neg_scale": "S",
    "cache_scale": "CS",
}
...
def get_hyperparameters_ranges(hyperparameters_combinations: pd.DataFrame) -> dict:
    return {
        col: hyperparameters_combinations[col].unique().tolist()
        for col in hyperparameters_combinations.drop(columns=["combination_idx"]).columns
    }
...
def print_table(
    ...
    bold_row = [bold_row] if isinstance(bold_row, int) else bold_row
    bold_column = [bold_column] if isinstance(bold_column, int) else bold_column
    grey_row = [grey_row] if isinstance(grey_row, int) else grey_row
    grey_column = [grey_column] if isinstance(grey_column, int) else grey_column
    table = plt.table(
        cellText=data,
        loc="center",
        cellLoc="center",
...
def clean_users_info(
    user_info: pd.DataFrame, include_base: bool, include_cache: bool
) -> pd.DataFrame:
    if not include_base:
...
def get_hyperparameters_combinations_table(
    val_upper_bounds: pd.DataFrame,
    optimizer_score: Score,
    best_global_hyperparameters_combinations_idxs: list,
    results_after_averaging_over_users: pd.DataFrame,
    ...
    for combination_idx in best_global_hyperparameters_combinations_idxs:
        row = (
            hyperparameters_combinations.loc[
                hyperparameters_combinations["combination_idx"] == combination_idx
            ]
            .values[0][1:]
            .tolist()
        )
        ...
        for score in list(PRINT_SCORES):
            row.append(
                format_number(
                    results_after_averaging_over_users.loc[
                        results_after_averaging_over_users["combination_idx"] == combination_idx,
                        f"val_{score.name.lower()}_mean",
                    ].values[0]
                )
            )
        data.append(row)
...
def print_fourth_page(
    pdf: PdfPages,
    hyperparameters_combinations_table: list,
    optimizer_score: Score,
    ...
    ax.text(
        0.5,
        1.1,
        "Validation Scores for Hyperparameters Combinations:\n",
        fontsize=16,
        ha="center",
        va="center",
        fontweight="bold",
    ...
    print_table(
        hyperparameters_combinations_table,
        [-0.14, -0.1, 1.25, 1.18],
        columns,
        len(hyperparameters) * [0.125] + len(PRINT_SCORES) * [0.15],
        bold_row=1,
...
def print_interesting_users(
    pdf: PdfPages,
    gv_score: Score,
    title: str,
    interesting_users_best_global_hyperparameters_combination_df: pd.DataFrame,
) -> None:
    fig, ax = plt.subplots(figsize=PLOT_CONSTANTS["FIG_SIZE"])
    ax.axis("off")
    ax.text(
        0.5,
        1.1,
        f"Users with {title} Performance for Best Global Hyperparameters Combination:\n",
        fontsize=12,
        ha="center",
        va="center",
        fontweight="bold",
    ...
    print_table(
        get_interesting_users_table(interesting_users_best_global_hyperparameters_combination_df),
        [-0.14, -0.12, 1.25, 1.19],
        columns,
        4 * [0.11] + len(Score) * [0.125],
...
def get_largest_performance_gain_table(
    largest_performance_gain_df: pd.DataFrame, gv_score: Score
) -> list:
    data = []
    for _, row in largest_performance_gain_df.iterrows():
        ...
        for score in PRINT_SCORES:
            ...
            if gain >= 0:
                data_row.append(
                    ("+" if SCORES_DICT[score]["increase_better"] else "-") + format_number(gain)
                )
            else:
                data_row.append(
                    ("-" if SCORES_DICT[score]["increase_better"] else "+") + format_number(-gain)
...
def print_largest_performance_gain(
    pdf: PdfPages,
    largest_performance_gain_df: pd.DataFrame,
    gv_score: Score,
    best_global_hyperparameters_combination_idx: int,
) -> None:
    fig, ax = plt.subplots(figsize=PLOT_CONSTANTS["FIG_SIZE"])
    ax.axis("off")
    ax.text(
        0.49,
        1.1,
        f"Users with Largest Performance Gain between Best Global Hyperparameters Combination {best_global_hyperparameters_combination_idx} and Best Individual:\n",
        fontsize=12,
        ha="center",
        va="center",
        fontweight="bold",
...
def get_hyperparameters_combinations_with_explicit_X_hyperparameter(
    hyperparameters: list, hyperparameters_combinations: pd.DataFrame
) -> pd.DataFrame:
    hyperparameters_without_X_hyperparameter = [
        hyperparameter
        for hyperparameter in hyperparameters
        if hyperparameter not in ["combination_idx", PLOT_CONSTANTS["X_HYPERPARAMETER"]]
    ]
    hyperparameters_combinations_with_explicit_X_hyperparameter = (
        hyperparameters_combinations.copy()
    )
    if len(hyperparameters_without_X_hyperparameter) > 0:
        hyperparameters_combinations_with_explicit_X_hyperparameter[
            "non_X_hyperparameter_combination_idx"
        ] = hyperparameters_combinations_with_explicit_X_hyperparameter.groupby(
            hyperparameters_without_X_hyperparameter
        ).ngroup()
    else:
        hyperparameters_combinations_with_explicit_X_hyperparameter[
...
def get_plot_df(
    ...
    hyperparameters_combinations_with_explicit_X_hyperparameter: pd.DataFrame,
) -> pd.DataFrame:
    relevant_score_columns = [f"{score.name.lower()}" for score in PLOT_CONSTANTS["PLOT_SCORES"]]
    relevant_train_columns = [f"train_{score}_mean" for score in relevant_score_columns] + [
        f"train_{score}_std" for score in relevant_score_columns
    ]
    relevant_val_columns = [f"val_{score}_mean" for score in relevant_score_columns] + [
        f"val_{score}_std" for score in relevant_score_columns
    ]
    ...
    plot_df = pd.merge(
        ...
        results_df[["combination_idx"] + relevant_columns],
        on="combination_idx",
    )
    plot_df = plot_df[
        ["non_X_hyperparameter_combination_idx", PLOT_CONSTANTS["X_HYPERPARAMETER"]]
        + relevant_columns
    ]
    return plot_df
    ...
    if results_tail_df is not None:
        if SCORES_DICT[score]["increase_better"]:
            best_X_hyperparameter_tail = results_tail_df.loc[
                results_tail_df[f"val_{score_name}_mean"].idxmax()
            ][X_hyperparameter]
            best_X_hyperparameter_tail_val_score = results_tail_df.loc[
                results_tail_df[f"val_{score_name}_mean"].idxmax()
            ][f"val_{score_name}_mean"]
        else:
            best_X_hyperparameter_tail = results_tail_df.loc[
                results_tail_df[f"val_{score_name}_mean"].idxmin()
            ][X_hyperparameter]
            best_X_hyperparameter_tail_val_score = results_tail_df.loc[
                results_tail_df[f"val_{score_name}_mean"].idxmin()
            ][f"val_{score_name}_mean"]
        ...
        (tail_line,) = axs.plot(
            results_tail_df[X_hyperparameter],
            tail_score,
            label="Tail Performance on Validation Set",
            alpha=alpha_plot,
            linewidth=line_width,
            color="red",
        )
...
def plot_hyperparameter(
    ...
    plot_scores, X_hyperparameter = (
        PLOT_CONSTANTS["PLOT_SCORES"],
        PLOT_CONSTANTS["X_HYPERPARAMETER"],
    )
...
def plot_hyperparameter_for_all_combinations(
    pdf: PdfPages,
    hyperparameters: list,
    hyperparameters_combinations_with_explicit_X_hyperparameter: pd.DataFrame,
    plot_df: pd.DataFrame,
    plot_tail_df: pd.DataFrame,
) -> None:
    X_hyperparameter = PLOT_CONSTANTS["X_HYPERPARAMETER"]
    hyperparameters_without_X_hyperparameter = [
        hyperparameter for hyperparameter in hyperparameters if hyperparameter != X_hyperparameter
    ]
    if len(hyperparameters_without_X_hyperparameter) == 0:
        included_combinations = sorted(
        ...
        for non_X_hyperparameter_combination_idx in range(non_X_hyperparameter_combinations):
            ...
            plot_df_subset = plot_df[
                plot_df["non_X_hyperparameter_combination_idx"]
                == non_X_hyperparameter_combination_idx
            ]
            plot_tail_df_subset = plot_tail_df[
                plot_tail_df["non_X_hyperparameter_combination_idx"]
                == non_X_hyperparameter_combination_idx
            ]
...
def get_user_folds_tables(
    user_results_before_averaging_over_folds: pd.DataFrame,
    user_results_after_averaging_over_folds: pd.DataFrame,
) -> tuple:
    n_tables = max([SCORES_DICT[score]["page"] for score in Score]) + 1
    columns_list = [["Fold"] for _ in range(n_tables)]
    for i, score in enumerate(list(Score)):
        score_abbreviation = SCORES_DICT[score]["abbreviation"]
        columns_list[SCORES_DICT[score]["page"]].append(score_abbreviation)
    val_first_row_list, train_first_row_list = [["μ"] for _ in range(n_tables)], [
        ["μ"] for _ in range(n_tables)
    ]
    for i, score in enumerate(list(Score)):
        val_first_row_list[SCORES_DICT[score]["page"]].append(
            format_number(
                user_results_after_averaging_over_folds[f"val_{score.name.lower()}_mean"].values[0]
            )
        )
        train_first_row_list[SCORES_DICT[score]["page"]].append(
            format_number(
                user_results_after_averaging_over_folds[f"train_{score.name.lower()}_mean"].values[
                    0
                ]
            )
        )
    val_table_list, train_table_list = [[val_first_row_list[i]] for i in range(n_tables)], [
        [train_first_row_list[i]] for i in range(n_tables)
    ]
    n_folds = len(user_results_before_averaging_over_folds)
    for fold_idx in range(n_folds):
        val_row_list, train_row_list = [[fold_idx] for _ in range(n_tables)], [
            [fold_idx] for _ in range(n_tables)
        ]
        for i, score in enumerate(list(Score)):
            val_row_list[SCORES_DICT[score]["page"]].append(
                format_number(
                    user_results_before_averaging_over_folds.loc[
                        user_results_before_averaging_over_folds["fold_idx"] == fold_idx,
                        f"val_{score.name.lower()}",
                    ].values[0]
                )
            )
            train_row_list[SCORES_DICT[score]["page"]].append(
                format_number(
                    user_results_before_averaging_over_folds.loc[
                        user_results_before_averaging_over_folds["fold_idx"] == fold_idx,
                        f"train_{score.name.lower()}",
                    ].values[0]
                )
            )
        for i in range(n_tables):
            val_table_list[i].append(val_row_list[i])
            train_table_list[i].append(train_row_list[i])
    val_last_row_list, train_last_row_list = [["σ"] for _ in range(n_tables)], [
        ["σ"] for _ in range(n_tables)
    ]
    for i, score in enumerate(list(Score)):
        val_last_row_list[SCORES_DICT[score]["page"]].append(
            format_number(
                user_results_after_averaging_over_folds[f"val_{score.name.lower()}_std"].values[0]
            )
        )
        train_last_row_list[SCORES_DICT[score]["page"]].append(
            format_number(
                user_results_after_averaging_over_folds[f"train_{score.name.lower()}_std"].values[0]
            )
        )
    for i in range(n_tables):
        val_table_list[i].append(val_last_row_list[i])
        train_table_list[i].append(train_last_row_list[i])
    return columns_list, val_table_list, train_table_list


def print_first_page_for_user(
    pdf: PdfPages,
    title: str,
    user_info_table: tuple,
    columns: list,
    val_table: list,
    train_table: list,
) -> None:
    ...
    ax.text(0.5, 0.945, "Validation Scores:", fontsize=13, ha="center", va="top", fontweight="bold")
    print_table(
        val_table,
        [-0.155, 0.45, 1.275, 0.46],
        columns,
        [0.1] + (len(columns) - 1) * [0.15],
        bold_row=1,
    )
    ax.text(0.5, 0.42, "Training Scores:", fontsize=13, ha="center", va="top", fontweight="bold")
    print_table(
        train_table,
        [-0.155, -0.075, 1.275, 0.46],
        columns,
        [0.1] + (len(columns) - 1) * [0.15],
        bold_row=1,
    )
...
def plot_hyperparameter_for_all_combinations_for_user(
    pdf: PdfPages,
    plot_df_user: pd.DataFrame,
    hyperparameters: list,
    hyperparameters_combinations_with_explicit_X_hyperparameter: pd.DataFrame,
    globally_optimal_X_hyperparameter: float,
    best_global_hyperparameters_combination_idx: int,
) -> None:
    X_hyperparameter = PLOT_CONSTANTS["X_HYPERPARAMETER"]
    ...
    if len(hyperparameters_without_X_hyperparameter) == 0:
        ...
        title += f"Combinations: {str(included_combinations)}.\n"
        plot_hyperparameter(
            pdf, plot_df_user, title, False, globally_optimal_X_hyperparameter, None
        )
    else:
        non_X_hyperparameter_combinations = len(
            hyperparameters_combinations_with_explicit_X_hyperparameter[
        ...
        for non_X_hyperparameter_combination_idx in range(non_X_hyperparameter_combinations):
            ...
            plot_df_subset = plot_df_user[
                plot_df_user["non_X_hyperparameter_combination_idx"]
                == non_X_hyperparameter_combination_idx
            ]
            for i, hyperparameter in enumerate(hyperparameters_without_X_hyperparameter):
            ...
            vertical_line = (
                globally_optimal_X_hyperparameter
                if best_global_hyperparameters_combination_idx in included_combinations
                else None
            )
            plot_hyperparameter(
                pdf, plot_df_subset, plot_subset_title + ".", False, vertical_line, None
            )
...
def get_classification_outcome(gt_label: int, prediction: int) -> Classification_Outcome:
    if gt_label == 1:
        return (
            ...
            if prediction == 1
            else Classification_Outcome.FALSE_NEGATIVE
        )
    elif gt_label == 0:
        return (
            Classification_Outcome.TRUE_NEGATIVE
            if prediction == 0
            else Classification_Outcome.FALSE_POSITIVE
        )


def merge_tfidf_scores(pos_train_scores: dict, neg_train_scores: dict) -> dict:
    pos_train_values_array, neg_train_values_array = np.array(
        list(pos_train_scores.values())
    ), np.array(list(neg_train_scores.values()))
    max_word_score = max(np.max(pos_train_values_array), np.max(neg_train_values_array))
    pos_train_values_array, neg_train_values_array = (
        pos_train_values_array / max_word_score,
        neg_train_values_array / max_word_score,
    )
...
def turn_predictions_into_df(predictions: list, ids: list, labels: list) -> pd.DataFrame:
    ...
    for paper_idx, val_prediction in enumerate(predictions):
        data.append(
            {
                "paper_id": ids[paper_idx],
                "gt_label": labels[paper_idx],
                "class_1_proba": val_prediction,
            }
        )
    predictions_df = pd.DataFrame(data)
    predictions_df["prediction"] = predictions_df["class_1_proba"].apply(
        lambda x: 1 if x >= 0.5 else 0
    )
    predictions_df["classification_outcome"] = predictions_df.apply(
        lambda row: get_classification_outcome(row["gt_label"], row["prediction"]), axis=1
    )
...
def get_papers_table_data(papers_texts: pd.DataFrame, outcome_df: pd.DataFrame) -> list:
    merged_df = outcome_df.merge(
        papers_texts[["paper_id", "title", "abstract"]], on="paper_id", how="left"
    )
    merged_df["title_clean"] = merged_df["title"].apply(
        lambda x: sanitize_latex(x).rstrip() if pd.notna(x) else ""
    )
    merged_df["abstract_clean"] = merged_df["abstract"].apply(
        lambda x: sanitize_latex(x).lstrip() if pd.notna(x) else ""
    )
...
def randomly_select_training_papers_label(
    papers_texts: pd.DataFrame,
    fold_predictions_df: pd.DataFrame,
    is_positive: bool,
    random_seed: int = 42,
) -> tuple:
    fold_predictions_subset_df = fold_predictions_df[
        fold_predictions_df["gt_label"] == int(is_positive)
    ]
    ...
    if n_train_papers_selection < n_train_papers_full:
        fold_predictions_subset_df = fold_predictions_subset_df.sample(
            n=n_train_papers_selection, random_state=random_seed
        )
    fold_predictions_subset_df = fold_predictions_subset_df.sort_values(
        by="class_1_proba", ascending=not is_positive
    )
    data = get_papers_table_data(papers_texts, fold_predictions_subset_df)
    return data, n_train_papers_full


def select_n_most_extreme_val_papers_outcome(
    papers_texts: pd.DataFrame,
    fold_predictions_df: pd.DataFrame,
    classification_outcome: Classification_Outcome,
    descending: bool,
) -> tuple:
    fold_predictions_df_classification_outcome = fold_predictions_df[
        fold_predictions_df["classification_outcome"] == classification_outcome
    ]
    n_val_papers_outcome_full = len(fold_predictions_df_classification_outcome)
    n_val_papers_outcome_selection = min(
        PLOT_CONSTANTS["N_PAPERS_IN_TOTAL"], n_val_papers_outcome_full
    )
    fold_predictions_df_classification_outcome = (
        fold_predictions_df_classification_outcome.nlargest(
            n_val_papers_outcome_selection, "class_1_proba"
        )
        if descending
        else fold_predictions_df_classification_outcome.nsmallest(
            n_val_papers_outcome_selection, "class_1_proba"
        )
    )
    data = get_papers_table_data(papers_texts, fold_predictions_df_classification_outcome)
    return data, n_val_papers_outcome_full


def select_n_most_extreme_val_pos_papers(
    papers_texts: pd.DataFrame, fold_predictions_df: pd.DataFrame
) -> tuple:
    ...
    fold_predictions_df_pos = fold_predictions_df_pos.nsmallest(
        n_val_pos_papers_selection, "class_1_proba"
    ).sort_values(by="class_1_proba", ascending=False)
    data = get_papers_table_data(papers_texts, fold_predictions_df_pos)
    return data, n_val_pos_papers_full


def train_wordclouds(
    train_ids: list, train_labels: list, val_ids: list, n_tfidf_features: int = 5000
) -> tuple:
    ...
    pos_train_mean_embedding, neg_train_mean_embedding = get_mean_embedding(
        v, pos_train_ids
    ), get_mean_embedding(v, neg_train_ids)
...
def generate_wordclouds(
    pdf: PdfPages,
    wc_pos_train_scores: dict,
    wc_neg_train_scores: dict,
    use_tfidf_coefs: bool,
    n_pos_train_papers_full: int = None,
    ...
    suptitle = (
        "Word Clouds based on Coefficients of TF-IDF Classifer."
        if use_tfidf_coefs
    ...
    def generate_wordcloud(word_scores, ax, title):
        ...
        wordcloud = WordCloud(
            ...
            max_words=50,
            random_state=42,
            colormap=colors.ListedColormap(color_palette),
...
def score_text_with_tfidf(text: str, tfidf_scores: dict) -> list:
    ...
    while i < len(words):
        ...
        if best_match:
            ...
            scored_words.extend(
                [(best_match_joined[i], best_score) for i in range(len(best_match_joined))]
            )
...
def plot_papers(pdf: PdfPages, table_data: list, table_name: str, word_scores: dict) -> None:
    ...
    for i in range(len(table_data)):
        ...
        title_scores, abstract_scores = score_text_with_tfidf(
            title, word_scores
        ), score_text_with_tfidf(abstract, word_scores)
...
def plot_training_papers_class(
    pdf: PdfPages,
    train_papers_selection: list,
    n_train_papers_full: int,
    word_scores: dict,
    is_positive: bool,
) -> None:
    n_train_papers_selection = len(train_papers_selection)
    train_papers_selection_n_pages = (
        (n_train_papers_selection - 1) // PLOT_CONSTANTS["N_PAPERS_PER_PAGE"]
    ) + 1
    for i in range(train_papers_selection_n_pages):
        title = (
            f"Random Selection of {n_train_papers_selection} {'Positively' if is_positive else 'Negatively'} "
            + f"Rated Training Papers (Total N = {n_train_papers_full}), Page {i+1} / {train_papers_selection_n_pages}."
        )
        plot_papers(
            pdf,
            train_papers_selection[
                i
                * PLOT_CONSTANTS["N_PAPERS_PER_PAGE"] : (i + 1)
                * PLOT_CONSTANTS["N_PAPERS_PER_PAGE"]
            ],
            title,
            word_scores,
        )
...
def plot_training_papers(
    pdf: PdfPages,
    pos_train_papers_selection: list,
    n_pos_train_papers_full: int,
    neg_train_papers_selection: list,
    n_neg_train_papers_full: int,
    word_scores: dict,
) -> None:
    plot_training_papers_class(
        pdf, pos_train_papers_selection, n_pos_train_papers_full, word_scores, True
    )
    plot_training_papers_class(
        pdf, neg_train_papers_selection, n_neg_train_papers_full, word_scores, False
    )


def plot_true_validation_papers_classification_outcome(
    pdf: PdfPages,
    true_val_papers_selection: list,
    n_true_val_papers_full: int,
    word_scores: dict,
    classification_outcome: Classification_Outcome,
) -> None:
    n_true_val_papers_selection = len(true_val_papers_selection)
    true_val_papers_selection_n_pages = (
        (n_true_val_papers_selection - 1) // PLOT_CONSTANTS["N_PAPERS_PER_PAGE"]
    ) + 1
    title = (
        f"Collection of the {n_true_val_papers_selection} Most Extremely Rated Validation "
...
def plot_true_validation_papers(
    pdf: PdfPages,
    true_pos_val_papers_selection: list,
    n_true_pos_val_papers_full: int,
    true_neg_val_papers_selection: list,
    n_true_neg_val_papers_full: int,
    word_scores: dict,
) -> None:
    plot_true_validation_papers_classification_outcome(
        pdf,
...
def plot_false_validation_papers_classification_outcome(
    pdf: PdfPages,
    false_val_papers_selection: list,
    n_false_val_papers_full: int,
    word_scores: dict,
    ...
    title = (
        f"Collection of the {n_false_val_papers_selection} Most Extremely Rated Validation "
...
def plot_false_validation_papers(
    pdf: PdfPages,
    false_pos_val_papers_selection: list,
    n_false_pos_val_papers_full: int,
    false_neg_val_papers_selection: list,
    n_false_neg_val_papers_full: int,
    word_scores: dict,
    cosine_similarities: dict,
    pos_train_papers_selection: list,
    neg_train_papers_selection: list,
) -> None:
    plot_false_validation_papers_classification_outcome(
        pdf,
    ...
    plot_false_validation_papers_classification_outcome(
        ...
        pos_train_papers_selection,
        neg_train_papers_selection,
        Classification_Outcome.FALSE_NEGATIVE,
    )


def plot_ranking_predictions(
    pdf: PdfPages,
    papers_selection: list,
    n_papers_full: int,
    pos_val_papers_selection: list,
    n_pos_val_papers_full: int,
    words_scores: dict,
    cosine_similarities: dict,
    pos_train_papers_selection: list,
    neg_train_papers_selection: list,
    is_negrated_ranking: bool,
) -> None:
    plot_ranking_overview(
        pdf, papers_selection, n_papers_full, pos_val_papers_selection, n_pos_val_papers_full
    )
    ...
    for i in range(n_papers_selection):
        plot_single_paper(
            ...
            pos_train_papers_selection,
            neg_train_papers_selection,
            samples=not is_negrated_ranking,
        )
...
def plot_ranking_overview(
        ...
        for i in range(papers_data.shape[0]):
            ...
            if formatted_number != "nan":
                papers_ax.text(
                    j,
                    i,
                    f"#{counter}:  {formatted_number}\n ID: {papers_selection[counter-1][1]}",
                    ha="center",
                    va="center",
                    color="black",
                    fontsize=8,
            ...
            counter += 1
    papers_ax.set_title(
        f"Model Probabilities for the {len(papers_selection)} Most Highly Rated Papers (Total N = {n_papers_full})",
        pad=3,
        fontsize=9,
        fontweight="bold",
    )
    ...
    pos_val_data = reshape_to_n_rows_10_cols(
        np.array([pos_val_paper[2] for pos_val_paper in pos_val_papers_selection]), 10
    )
        ...
        for i in range(pos_val_data.shape[0]):
            ...
            if formatted_number != "nan":
                pos_val_ax.text(
                    j,
                    i,
                    f"#{counter}:  {formatted_number}\n ID: {pos_val_papers_selection[counter-1][1]}",
                    ha="center",
                    va="center",
                    color="black",
                    fontsize=8,
            ...
            counter += 1
    pos_val_ax.set_title(
        f"Model Probabilities for the {len(pos_val_papers_selection)} Least Highly Rated Validation GT-Positives (Total N = {n_pos_val_papers_full})",
        pad=3,
        fontsize=9,
        fontweight="bold",
    )
...
def preprocess_cosine_similarities(
    paper_id: int,
    cosine_similarities: dict,
    pos_train_papers_selection: list,
    neg_train_papers_selection: list,
    ...
    ids_to_idxs = {
        ...
        for idx, paper_id in enumerate(
            cosine_similarities["posrated_ids"] + cosine_similarities["negrated_ids"]
        )
    }
    if samples:
        ids_to_idxs_samples = {
            paper_id: idx
            for idx, paper_id in enumerate(cosine_similarities["negative_samples_ids"])
        }
        relevant_row = cosine_similarities["negative_samples_cosine_similarities"][
            ids_to_idxs_samples[paper_id]
        ]
    else:
        relevant_row = cosine_similarities["rated_cosine_similarities"][ids_to_idxs[paper_id]]
    pos_relevant_row, neg_relevant_row = np.zeros(len(pos_train_papers_selection)), np.zeros(
        len(neg_train_papers_selection)
    )
    pos_relevant_ids_row, neg_relevant_ids_row = np.zeros(
        len(pos_train_papers_selection), dtype=np.int32
    ), np.zeros(len(neg_train_papers_selection), dtype=np.int32)
...
def plot_single_paper(
    ...
    preprocess_cosine = preprocess_cosine_similarities(
        table_data[1],
        cosine_similarities,
        pos_train_papers_selection,
        neg_train_papers_selection,
    ...
    pos_relevant_row, pos_relevant_ids_row, neg_relevant_row, neg_relevant_ids_row = (
        preprocess_cosine
    )
    plt.rcParams["text.usetex"] = False
    fig, ax = plt.subplots(figsize=PLOT_CONSTANTS["FIG_SIZE"])
    ...
    pos_ax.set_title(
        f"Cosine Similarities to the Selection of {len(pos_relevant_row)} positively rated Papers:",
        pad=3,
        fontsize=9,
        fontweight="bold",
    )
    ...
    for j in range(neg_data.shape[1]):
        for i in range(neg_data.shape[0]):
            ...
            counter += 1
    neg_ax.set_title(
        f"Cosine Similarities to the Selection of {len(neg_relevant_row)} negatively rated Papers:",
        pad=3,
        fontsize=9,
        fontweight="bold",
    )
...
def get_tfidf_coefs_scores(feature_names: list, tfidf_coefs: list) -> tuple:
    ...
    largest_pos_tfidf_coef, largest_neg_tfidf_coef = np.max(tfidf_coefs[tfidf_coefs >= 0]), np.min(
        tfidf_coefs[tfidf_coefs < 0]
    )
    ...
    wc_pos_train_scores = {
        feature_names[i]: tfidf_coefs[i] for i in range(len(feature_names)) if tfidf_coefs[i] >= 0
    }
    wc_neg_train_scores = {
        feature_names[i]: tfidf_coefs[i] for i in range(len(feature_names)) if tfidf_coefs[i] < 0
    }
    return (
        ...
        wc_neg_train_scores,
        wc_words_scores,
        largest_pos_tfidf_coef,
        largest_neg_tfidf_coef,
    )
