...
if __name__ == "__main__":
    ...
    embeddings_output_folder = (
        ...
        / "after_pca"
        / f"{EMBEDDINGS_INPUT_FOLDER.stem}_{PCA_DIM}"
    )
    print(
        f"Loaded embeddings matrix of shape {embeddings_matrix.shape} for PCA down to {PCA_DIM} dimensions."
    )
    ...
    if APPLY_ZSCORE:
        ...
        print(
            f"Z-score normalization successful. Took {time.time() - start_time:.2f} seconds. New Shape: {embeddings_matrix.shape}."
        )
    ...
    if apply_pca:
        ...
        pca_mean = pca.mean_
        print(
            f"PCA successful. Took {time.time() - start_time:.2f} seconds. New Shape: {embeddings_matrix.shape}."
        )
    else:
        print("Skipping PCA..")
    ...
    if APPLY_L2NORM:
        ...
        print(
            f"L2 normalization successful. Took {time.time() - start_time:.2f} seconds. New Shape: {embeddings_matrix.shape}."
        )
    ...
    shutil.copy2(
        EMBEDDINGS_INPUT_FOLDER / "abs_paper_ids_to_idx.pkl",
        embeddings_output_folder / "abs_paper_ids_to_idx.pkl",
    )
