...
from .papers_categories_dicts import PAPERS_CATEGORIES, PapersCategories
...
def normalize_embedding(glove_embedding: np.ndarray, normalization: str = "l2_unit") -> np.ndarray:
    if normalization == "none":
        ...
        proportionality = glove_embedding.shape[0] / (
            glove_embedding.shape[0] + embedding.matrix.shape[1]
        )
...
def get_glove_categories_embeddings(
    categories_to_glove: dict, dim: int = 100, normalization: str = "l2_unit"
) -> dict:
    ...
    for category, words in categories_to_glove.items():
        ...
        glove_categories_embeddings[category] = normalize_embedding(
            glove_category_embedding, normalization
        )
    return glove_categories_embeddings


def get_papers_ids_to_categories(
    papers_ids_to_categories_original: dict, original_categories_to_categories: dict
) -> dict:
...
def attach_papers_categories(
    ...
    glove_categories_embeddings = get_glove_categories_embeddings(
        papers_categories.categories_to_glove, dim, normalization
    )
