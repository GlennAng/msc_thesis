...
def derive_score(
    score: Score, user_scores: list, scores_indices_dict: dict, validation: bool
) -> float:
    if score == Score.BALANCED_ACCURACY:
        recall = user_scores[
            scores_indices_dict[f"{'val' if validation else 'train'}_{Score.RECALL.name.lower()}"]
        ]
        specificity = user_scores[
            scores_indices_dict[
                f"{'val' if validation else 'train'}_{Score.SPECIFICITY.name.lower()}"
            ]
        ]
        return (recall + specificity) / 2
    elif score == Score.F1_SCORE:
        precision = user_scores[
            scores_indices_dict[
                f"{'val' if validation else 'train'}_{Score.PRECISION.name.lower()}"
            ]
        ]
        recall = user_scores[
            scores_indices_dict[f"{'val' if validation else 'train'}_{Score.RECALL.name.lower()}"]
        ...
        precision_samples = user_scores[
            scores_indices_dict[
                f"{'val' if validation else 'train'}_{Score.PRECISION_SAMPLES.name.lower()}"
            ]
        ]
        recall_samples = user_scores[
            scores_indices_dict[f"{'val' if validation else 'train'}_{Score.RECALL.name.lower()}"]
        ]
...
def get_ranking_scores_split(
    ...
    ranking_scores_without_bottom_1_pos = [
        score for score in RANKING_SCORES if score not in BOTTOM_1_POS_SCORES
    ]
...
def get_category_scores(
    ...
    l1_train_rated_pos, l1_train_rated_neg = pd.Series(
        categories_dict["l1_train_rated"][train_rated_pos_mask]
    ), pd.Series(categories_dict["l1_train_rated"][~train_rated_pos_mask])
    l1_val_pos, l1_val_neg = pd.Series(categories_dict["l1_val"][val_pos_mask]), pd.Series(
        categories_dict["l1_val"][~val_pos_mask]
    )
    l2_train_rated_pos, l2_train_rated_neg = pd.Series(
        categories_dict["l2_train_rated"][train_rated_pos_mask]
    ), pd.Series(categories_dict["l2_train_rated"][~train_rated_pos_mask])
    l2_val_pos, l2_val_neg = pd.Series(categories_dict["l2_val"][val_pos_mask]), pd.Series(
        categories_dict["l2_val"][~val_pos_mask]
    )

    l1l2_train_rated_pos = pd.Series(
        [f"{l1}_{l2}" for l1, l2 in zip(l1_train_rated_pos, l2_train_rated_pos)]
    )
    l1l2_train_rated_neg = pd.Series(
        [f"{l1}_{l2}" for l1, l2 in zip(l1_train_rated_neg, l2_train_rated_neg)]
    )
    ...
    for category_score in CATEGORY_SCORES:
        category_scores[f"train_{category_score.name.lower()}"] = get_category_score(
            category_score,
            l1_train_rated_pos,
            l1_train_rated_neg,
            l1l2_train_rated_pos,
            l1l2_train_rated_neg,
        )
        category_scores[f"val_{category_score.name.lower()}"] = get_category_score(
            category_score, l1_val_pos, l1_val_neg, l1l2_val_pos, l1l2_val_neg
        )
