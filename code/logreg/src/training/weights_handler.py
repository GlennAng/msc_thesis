...
def load_hyperparameter_range(hyperparameter_range_raw: object) -> list:
    ...
    if isinstance(hyperparameter_range_raw, list):
        ...
        if hyperparameter_range_raw.startswith("linspace") or hyperparameter_range_raw.startswith(
            "logspace"
        ):
            hyperparameter_range = eval(f"np.{hyperparameter_range_raw}")
        elif hyperparameter_range_raw.startswith(
            "np.linspace"
        ) or hyperparameter_range_raw.startswith("np.logspace"):
...
def get_weights_group_from_arg(weights_group_arg: str) -> Weights_Group:
    ...
    if weights_group_arg not in valid_weights_group_args:
        raise ValueError(
            f"Invalid Weights Group {weights_group_arg} in 'weights'. Possible values: {valid_weights_group_args}."
        )
...
TRANSFORMATION_WEIGHTS_VW = {
    Weights_Scheme.BOTH_CONSTANT_VW,
    Weights_Scheme.BOTH_CUBE_ROOT_VW,
    Weights_Scheme.BOTH_LOGARITHM_VW,
    Weights_Scheme.BOTH_SQUARE_ROOT_VW,
    Weights_Scheme.BOTH_LINEAR_VW,
    Weights_Scheme.BOTH_SQUARE_VW,
    Weights_Scheme.BOTH_EXPONENTIAL_VW,
    Weights_Scheme.RATED_CONSTANT_VW,
    Weights_Scheme.RATED_CUBE_ROOT_VW,
    Weights_Scheme.RATED_LOGARITHM_VW,
    Weights_Scheme.RATED_SQUARE_ROOT_VW,
    Weights_Scheme.RATED_LINEAR_VW,
    Weights_Scheme.RATED_SQUARE_VW,
    Weights_Scheme.RATED_EXPONENTIAL_VW,
}
TRANSFORMATION_WEIGHTS_HYP = {
    Weights_Scheme.BOTH_CONSTANT_HYP,
    Weights_Scheme.BOTH_CUBE_ROOT_HYP,
    Weights_Scheme.BOTH_LOGARITHM_HYP,
    Weights_Scheme.BOTH_SQUARE_ROOT_HYP,
    Weights_Scheme.BOTH_LINEAR_HYP,
    Weights_Scheme.BOTH_SQUARE_HYP,
    Weights_Scheme.BOTH_EXPONENTIAL_HYP,
    Weights_Scheme.RATED_CONSTANT_HYP,
    Weights_Scheme.RATED_CUBE_ROOT_HYP,
    Weights_Scheme.RATED_LOGARITHM_HYP,
    Weights_Scheme.RATED_SQUARE_ROOT_HYP,
    Weights_Scheme.RATED_LINEAR_HYP,
    Weights_Scheme.RATED_SQUARE_HYP,
    Weights_Scheme.RATED_EXPONENTIAL_HYP,
}
...
def get_weights_scheme_from_arg(weights_scheme_arg: str) -> Weights_Scheme:
    ...
    if weights_scheme_arg not in valid_weights_scheme_args:
        raise ValueError(
            f"Invalid Weights Scheme {weights_scheme_arg} in 'weights'. Possible values: {valid_weights_scheme_args}."
        )
...
class Weights_Handler:
    ...
    def verify_weights_schemes(self, weights_schemes: dict, weights_arg: str) -> None:
        ...
        if weights_schemes[Weights_Group.GLOBAL] is not None:
            if any(
                [
                    weights_schemes[Weights_Group.POS] is not None,
                    weights_schemes[Weights_Group.NEG] is not None,
                    weights_schemes[Weights_Group.POSNEG] is not None,
                ]
            ):
                raise ValueError(
                    f"Invalid Weights Command '{weights_arg}' in 'weights'. If 'global' is set, 'pos' and 'neg' must be None."
                )
        else:
            if weights_schemes[Weights_Group.POSNEG] is not None:
                if any(
                    ...
                    raise ValueError(
                        f"Invalid Weights Command '{weights_arg}' in 'weights'. If 'posneg' is set, 'pos' and 'neg' must be None."
                    )
            else:
                if (
                    weights_schemes[Weights_Group.POS] is not None
                    and weights_schemes[Weights_Group.NEG] is None
                ):
                    raise ValueError(
                        f"Invalid Weights Command '{weights_arg}' in 'weights'. If 'pos' is set, 'neg' must be set."
                    )
                elif (
                    weights_schemes[Weights_Group.POS] is None
                    and weights_schemes[Weights_Group.NEG] is not None
                ):
                    raise ValueError(
                        f"Invalid Weights Command '{weights_arg}' in 'weights'. If 'neg' is set, 'pos' must be set."
                    )
    ...
    def load_weights_hyperparameters(self, config: dict) -> None:
        ...
        if self.global_weights_scheme is not None:
            weights_hyperparameters_ranges.update(
                self.load_weights_hyperparameters_global(self.global_weights_scheme, config)
            )
        else:
            weights_hyperparameters_ranges.update(
                self.load_weights_hyperparameters_label(
                    self.pos_weights_scheme, is_positive=True, config=config
                )
            )
            weights_hyperparameters_ranges.update(
                self.load_weights_hyperparameters_label(
                    self.neg_weights_scheme, is_positive=False, config=config
                )
            )
    ...
    def load_weights_hyperparameters_label(
        self, weights_scheme: Weights_Scheme, is_positive: bool, config: dict
    ) -> dict:
        if weights_scheme == Weights_Scheme.UNWEIGHTED:
            ...
            transformation_weights_hyp_str = (
                f"weights_{'pos' if is_positive else 'neg'}rated_importance"
            )
            return {
                transformation_weights_hyp_str: load_hyperparameter_range(
                    config[transformation_weights_hyp_str]
                )
            }

    def load_weights_for_user(
        self,
        ...
        if self.global_weights_scheme is not None:
            ...
            w_p, w_b = self.load_weights_for_user_label(
                hyperparameters,
                hyperparameters_combination,
                voting_weight,
                train_posrated_n,
            ...
            w_n, w_z, w_c = self.load_weights_for_user_label(
                hyperparameters,
                hyperparameters_combination,
                voting_weight,
                train_posrated_n,
        ...
        if "weights_neg_scale" in hyperparameters:
            ...
            w_z = (
                w_z * hyperparameters_combination[hyperparameters["weights_neg_scale"]]
                if w_z is not None
                else None
            )
        return w_p, w_n, w_b, w_z, w_c

    def load_weights_for_user_global(
        self,
        hyperparameters: dict,
        hyperparameters_combination: tuple,
        voting_weight: float,
        ...
        if self.global_weights_scheme == Weights_Scheme.CACHE_V:
            ...
            if "weights_cache_v" in hyperparameters:
                ...
                v3 = 1.0 - v1 - v2
                shared_scalar = correction / (
                    v1 * train_negrated_n + v2 * zerorated_n + v3 * cache_n
                )
            ...
            w_c = (
                hyperparameters_combination[hyperparameters["cache_scale"]] / cache_n
                if cache_n > 0
                else 0.0
            )
            return w_p, w_n, None, None, w_c

    def load_weights_for_user_label(
        self,
        hyperparameters: dict,
        hyperparameters_combination: tuple,
        voting_weight: float,
        ...
        if weights_scheme == Weights_Scheme.UNWEIGHTED:
            ...
            weights_scheme in TRANSFORMATION_WEIGHTS_VW
            or weights_scheme in TRANSFORMATION_WEIGHTS_HYP
        ):
            if is_positive:
                subclass1_n, subclass2_n = train_posrated_n, base_n
                scalar = (
                    hyperparameters_combination[hyperparameters["weights_posrated_importance"]]
                    if weights_scheme in TRANSFORMATION_WEIGHTS_HYP
                    else voting_weight
                )
            else:
                subclass1_n, subclass2_n = train_negrated_n, cache_n
                scalar = (
                    hyperparameters_combination[hyperparameters["weights_negrated_importance"]]
                    if weights_scheme in TRANSFORMATION_WEIGHTS_HYP
                    else voting_weight
                )
            if subclass2_n > 0:
                subclass1_weight, subclass2_weight = (
                    self.load_transformation_weights_for_user_label(
                        weights_scheme, scalar, subclass1_n, subclass2_n
                    )
                )
            else:
    ...
    def load_transformation_weights_for_user_label(
        self, weights_scheme: Weights_Scheme, scalar: float, subclass1_n: int, subclass2_n: int
    ) -> tuple:
        subclass1_transformation, subclass2_transformation = self.get_transformation_functions(
            weights_scheme
        )
        denominator = scalar * subclass1_transformation(subclass1_n) + (
            1.0 - scalar
        ) * subclass2_transformation(subclass2_n)
        subclass1_weight = (
            scalar * subclass1_transformation(subclass1_n) / (denominator * subclass1_n)
            if subclass1_n > 0
            else 0
        )
        subclass2_weight = (
            (1.0 - scalar) * subclass2_transformation(subclass2_n) / (denominator * subclass2_n)
            if subclass2_n > 0
            else 0
        )
