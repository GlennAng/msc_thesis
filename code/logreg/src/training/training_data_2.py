...
def get_user_categories_ratios(
    ...
    categories_to_exclude: list = None,
) -> dict:
    if categories_ratios is None:
        categories_ratios = get_categories_ratios()
    if categories_to_exclude is None or len(categories_to_exclude) == 0:
        return categories_ratios
    for cat in categories_to_exclude:
        if cat in categories_ratios:
            del categories_ratios[cat]
    total_ratio = sum(categories_ratios.values())
    user_categories_ratios = {cat: ratio / total_ratio for cat, ratio in categories_ratios.items()}
    assert sum(user_categories_ratios.values()) == 1.0, (
        "User categories ratios must sum to 1.0."
    )
