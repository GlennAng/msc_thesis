...
def save_users_embeddings(
    users_embeddings: dict, users_embeddings_folder: Path, single_val_session: bool
) -> None:
    users_embeddings_folder.mkdir(parents=True, exist_ok=True)
    if single_val_session:
        ...
        np.save(users_embeddings_folder / "users_coefs.npy", users_coefs)
        with open(users_embeddings_folder / "users_coefs_ids_to_idxs.pkl", "wb") as f:
            pickle.dump(users_coefs_ids_to_idxs, f)
    else:
        with open(users_embeddings_folder / "users_embeddings.pkl", "wb") as f:
            pickle.dump(users_embeddings, f)
    print(f"Users embeddings saved to {users_embeddings_folder}.")
...
def load_users_scores(path: Path) -> dict:
    if not isinstance(path, Path):
        path = Path(path).resolve()
    with open(path / "users_scores.pkl", "rb") as f:
