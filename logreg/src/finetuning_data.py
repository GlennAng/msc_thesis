...
class FinetuningDataset(Dataset):
    def __init__(self, user_idx_tensor : torch.Tensor, paper_id_tensor : torch.Tensor, label_tensor : torch.Tensor, input_ids_tensor : torch.Tensor, attention_mask_tensor : torch.Tensor,
                       category_idx_tensor : torch.Tensor, category_idx_l2_tensor : torch.Tensor = None) -> None:
        ...
        self.category_idx_l2_tensor = category_idx_l2_tensor
        if category_idx_l2_tensor is not None:
            assert len(category_idx_l2_tensor) == len(user_idx_tensor)
    ...
    def __getitem__(self, idx : int) -> dict:
        ...
        if self.category_idx_l2_tensor is not None:
            batch["category_idx_l2"] = self.category_idx_l2_tensor[idx]
        return batch
...
def create_dataset(dataset : dict) -> FinetuningDataset:
    ...
    category_idx_l2_tensor = dataset["category_idx_l2"] if "category_idx_l2" in dataset else None
    return FinetuningDataset(user_idx_tensor = user_idx_tensor, paper_id_tensor = paper_id_tensor, label_tensor = label_tensor,
                             ...
                             category_idx_l2_tensor = category_idx_l2_tensor)
...
def get_train_ratings_dataloader(args_dict : dict, users_embeddings_ids_to_idxs : dict, papers_ids_to_categories_idxs : dict, papers_ids_to_categories_idxs_l2 : dict) -> DataLoader:
    train_ratings = create_dataset(load_dataset("train", users_embeddings_ids_to_idxs, papers_ids_to_categories_idxs, papers_ids_to_categories_idxs_l2))
...
class TrainNegativeSamplesDataset(Dataset):
    ...
    def __init__(self, paper_id_tensor : torch.tensor, category_idx_tensor : torch.tensor, category_idx_l2_tensor : torch.tensor = None) -> None:
    ...
    def __getitem__(self, idx : int) -> dict:
        batch = {"paper_id" : self.paper_id_tensor[idx], "category_idx" : self.category_idx_tensor[idx]}
        if self.category_idx_l2_tensor is not None:
            batch["category_idx_l2"] = self.category_idx_l2_tensor[idx]
        return batch
...
class TrainNegativeSamplesBatchSampler(BatchSampler):
    ...
    def run_test(self, batch : dict) -> bool:
        ...
        if "category_idx_l2" in batch:
            assert len(batch["category_idx_l2"]) == len_batch
...
class TrainNegativeSamplesBatchProcesser:
    ...
    def collate_fn(self, batch : dict) -> dict:
        ...
        category_idx_l2_tensor = torch.stack([b["category_idx_l2"] for b in batch]) if "category_idx_l2" in batch[0] else None
        assert len(paper_id_tensor) == len(category_idx_tensor)
        ...
        batch = {"paper_id" : paper_id_tensor, "category_idx" : category_idx_tensor, "input_ids" : batch_tokenized_papers["input_ids"], 
                "attention_mask" : batch_tokenized_papers["attention_mask"]}
        if category_idx_l2_tensor is not None:
            assert len(category_idx_l2_tensor) == len(paper_id_tensor)
            batch["category_idx_l2"] = category_idx_l2_tensor
        return batch

def get_train_negative_samples_dataloader(n_train_negative_samples : int, papers_ids_to_categories_idxs : dict, n_batches_total : int, seed : int, 
                                          papers_ids_to_categories_idxs_l2 : torch.tensor = None) -> DataLoader:
    ...
    paper_id_tensor, category_idx_tensor, category_idx_l2_tensor = [], [], []
    for paper_id, category_idx in papers_ids_to_categories_idxs.items():
        if category_idx in relevant_categories_idxs and paper_id not in papers_to_exclude:
            ...
            if papers_ids_to_categories_idxs_l2 is not None:
                category_idx_l2_tensor.append(papers_ids_to_categories_idxs_l2[paper_id])
    paper_id_tensor, category_idx_tensor = torch.tensor(paper_id_tensor), torch.tensor(category_idx_tensor)
    category_idx_l2_tensor = torch.tensor(category_idx_l2_tensor) if category_idx_l2_tensor else None
    train_negative_samples_dataset = TrainNegativeSamplesDataset(paper_id_tensor, category_idx_tensor, category_idx_l2_tensor)
...
def load_val_data(users_embeddings_ids_to_idxs : dict, papers_ids_to_categories_idxs : dict, papers_ids_to_categories_idxs_l2 : dict = None) -> dict:
    val_data = {}
    ...
    val_data["val_ratings"] = create_dataset(load_dataset("val", users_embeddings_ids_to_idxs, papers_ids_to_categories_idxs, papers_ids_to_categories_idxs_l2))
    val_data["val_negative_samples"] = load_papers(path = "val_negative_samples", papers_ids_to_categories_idxs = papers_ids_to_categories_idxs, 
                                                   papers_ids_to_categories_idxs_l2 = papers_ids_to_categories_idxs_l2)
    return val_data
