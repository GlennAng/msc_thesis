...
class User_Visualizer:
    ...
    def visualize_user_info(self, pdf: PdfPages) -> None:
        ...
        columns_list, val_table_list, train_table_list = get_user_folds_tables(self.user_results_before_averaging_over_folds, self.user_results_after_averaging_over_folds)
        ...
        for i in range(n_tables):
            columns, val_table, train_table = columns_list[i], val_table_list[i], train_table_list[i]
            table_title = f"{i + 1}/{n_tables}   -   " + title
            print_first_page_for_user(pdf, table_title, user_info_table, columns, val_table, train_table)
    ...
    def visualize_papers(self, pdf: PdfPages, fold_idx: int, wc_words_scores: dict) -> None:
        ...
        negrated_ranking_selection = get_papers_table_data(papers_texts, negrated_ranking_predictions_df)
        ...
        negative_samples_selection = get_papers_table_data(papers_texts, negative_samples_predictions_df)
        
        pos_train_papers_selection, n_pos_train_papers_full = randomly_select_training_papers_label(papers_texts, fold_train_predictions_df, True)
        neg_train_papers_selection, n_neg_train_papers_full = randomly_select_training_papers_label(papers_texts, fold_train_predictions_df, False)
        true_pos_val_papers_selection, n_true_pos_val_papers_full = select_n_most_extreme_val_papers_outcome(papers_texts, fold_val_predictions_df, Classification_Outcome.TRUE_POSITIVE, True)
        true_neg_val_papers_selection, n_true_neg_val_papers_full = select_n_most_extreme_val_papers_outcome(papers_texts, fold_val_predictions_df, Classification_Outcome.TRUE_NEGATIVE, False)
        false_pos_val_papers_selection, n_false_pos_val_papers_full = select_n_most_extreme_val_papers_outcome(papers_texts, fold_val_predictions_df, Classification_Outcome.FALSE_POSITIVE, True)
        false_neg_val_papers_selection, n_false_neg_val_papers_full = select_n_most_extreme_val_papers_outcome(papers_texts, fold_val_predictions_df, Classification_Outcome.FALSE_NEGATIVE, False)
        pos_val_papers_selection, n_pos_val_papers_full = select_n_most_extreme_val_pos_papers(papers_texts, fold_val_predictions_df)
...
if __name__ == "__main__":
    ...
    for user_id in args["users_ids"]:
        ...
        user_predictions_path = gv.folder / "users_predictions" / f"user_{user_id}" / "user_predictions.json"
