...
from compute_tfidf import get_mean_embedding, train_vectorizer_for_user
...
def get_best_global_hyperparameters_combination_tables(best_global_hyperparameters_combination_df: pd.DataFrame, tail_users: np.ndarray, high_votes_users: np.ndarray, 
...
def print_fifth_page(pdf: PdfPages, title: str, legend_text: str, best_global_hyperparameters_combination_table: list, optimizer_row: int, save_path: Path = None) -> None:
    fig, ax = plt.subplots(figsize = PLOT_CONSTANTS["FIG_SIZE"])
    ax.axis("off")
...
def plot_hyperparameter_for_all_combinations(pdf: PdfPages, hyperparameters: list, hyperparameters_combinations_with_explicit_X_hyperparameter: pd.DataFrame, 
                                             plot_df: pd.DataFrame, plot_tail_df: pd.DataFrame) -> None:
...
def get_user_info_table(user_info: pd.DataFrame) -> tuple:
    columns = ["Number of Positively Rated", "Number of Negatively Rated", "Number of Base", "Number of Cache"]
    data = [[user_info["n_posrated"].iloc[0], user_info["n_negrated"].iloc[0], user_info["n_base"].iloc[0], user_info["n_cache"].iloc[0]]]
    return columns, data
...
def get_user_folds_tables(user_results_before_averaging_over_folds: pd.DataFrame, user_results_after_averaging_over_folds: pd.DataFrame) -> tuple:
    ...
    val_first_row_list, train_first_row_list = [["μ"] for _ in range(n_tables)], [["μ"] for _ in range(n_tables)]
    for i, score in enumerate(list(Score)):
        val_first_row_list[SCORES_DICT[score]["page"]].append(format_number(user_results_after_averaging_over_folds[f"val_{score.name.lower()}_mean"].values[0]))
        train_first_row_list[SCORES_DICT[score]["page"]].append(format_number(user_results_after_averaging_over_folds[f"train_{score.name.lower()}_mean"].values[0]))
    val_table_list, train_table_list = [[val_first_row_list[i]] for i in range(n_tables)], [[train_first_row_list[i]] for i in range(n_tables)]
    n_folds = len(user_results_before_averaging_over_folds)
    for fold_idx in range(n_folds):
        val_row_list, train_row_list = [[fold_idx] for _ in range(n_tables)], [[fold_idx] for _ in range(n_tables)]
        for i, score in enumerate(list(Score)):
            val_row_list[SCORES_DICT[score]["page"]].append(format_number(user_results_before_averaging_over_folds.loc[user_results_before_averaging_over_folds["fold_idx"] == fold_idx, 
                                                                        f"val_{score.name.lower()}"].values[0]))
            train_row_list[SCORES_DICT[score]["page"]].append(format_number(user_results_before_averaging_over_folds.loc[user_results_before_averaging_over_folds["fold_idx"] == fold_idx, 
                                                                        f"train_{score.name.lower()}"].values[0]))
        for i in range(n_tables):
            val_table_list[i].append(val_row_list[i])
            train_table_list[i].append(train_row_list[i])
    val_last_row_list, train_last_row_list = [["σ"] for _ in range(n_tables)], [["σ"] for _ in range(n_tables)]
    for i, score in enumerate(list(Score)):
        val_last_row_list[SCORES_DICT[score]["page"]].append(format_number(user_results_after_averaging_over_folds[f"val_{score.name.lower()}_std"].values[0]))
        train_last_row_list[SCORES_DICT[score]["page"]].append(format_number(user_results_after_averaging_over_folds[f"train_{score.name.lower()}_std"].values[0]))
    for i in range(n_tables):
...
def get_papers_table_data(papers_texts: pd.DataFrame, outcome_df: pd.DataFrame) -> list:
    merged_df = outcome_df.merge(papers_texts[["paper_id", "title", "abstract"]], on = "paper_id", how = "left")
    merged_df["title_clean"] = merged_df["title"].apply(lambda x: sanitize_latex(x).rstrip() if pd.notna(x) else "")
    merged_df["abstract_clean"] = merged_df["abstract"].apply(lambda x: sanitize_latex(x).lstrip() if pd.notna(x) else "")
    ...
    for i, (_, row) in enumerate(merged_df.iterrows(), 1):
        table_data.append([i, int(row["paper_id"]), row["class_1_proba_formatted"], row["title_clean"], row["abstract_clean"]])
    return table_data

def randomly_select_training_papers_label(papers_texts: pd.DataFrame, fold_predictions_df: pd.DataFrame, is_positive: bool, random_seed: int = 42) -> tuple:
...
def select_n_most_extreme_val_papers_outcome(papers_texts: pd.DataFrame, fold_predictions_df: pd.DataFrame, classification_outcome: Classification_Outcome, descending: bool) -> tuple:
...
def select_n_most_extreme_val_pos_papers(papers_texts: pd.DataFrame, fold_predictions_df: pd.DataFrame) -> tuple:
...
def plot_ranking_overview(pdf: PdfPages, papers_selection: list, n_papers_full: int, pos_val_papers_selection: list, n_pos_val_papers_full: int) -> None:
    ...
    for j in range(papers_data.shape[1]):
        for i in range(papers_data.shape[0]):
            ...
            counter += 1     
    papers_ax.set_title(f'Model Probabilities for the {len(papers_selection)} Most Highly Rated Papers (Total N = {n_papers_full})', 
                        pad = 3, fontsize = 9, fontweight = 'bold')
    ...
    for j in range(pos_val_data.shape[1]):
        for i in range(pos_val_data.shape[0]):
            ...
            counter += 1
    pos_val_ax.set_title(f'Model Probabilities for the {len(pos_val_papers_selection)} Least Highly Rated Validation GT-Positives (Total N = {n_pos_val_papers_full})', 
                        pad = 3, fontsize = 9, fontweight = 'bold')
...
def plot_ranking_overview(pdf: PdfPages, negative_samples_selection: list, n_negative_samples_full: int, pos_val_papers_selection: list, n_pos_val_papers_full: int) -> None:
    ...
    for j in range(neg_samples_data.shape[1]):
        for i in range(neg_samples_data.shape[0]):
            ...
            counter += 1     
    neg_samples_ax.set_title(f'Model Probabilities for the {len(negative_samples_selection)} Most Highly Rated Negative Samples (Total N = {n_negative_samples_full})', 
                            pad = 3, fontsize = 9, fontweight = 'bold')
