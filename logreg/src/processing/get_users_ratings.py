...
def get_train_test_split(users_ratings: pd.DataFrame, test_size: float, random_state: int, stratify: bool = True) -> pd.DataFrame:
    ...
    result_df = users_ratings.groupby("user_id", group_keys = False).apply(split_user_data)
...
def get_users_ratings(users_selection: str, evaluation: Evaluation, test_size: float, max_users: int = None, users_random_state: int = 42, min_n_posrated: int = 0, min_n_negrated: int = 0,
                      stratify: bool = True, take_complement: bool = False, users_mapped: bool = False, model_random_state: int = 42) -> tuple:
...
if __name__ == "__main__":
    ...
    get_users_ratings(users_selection = "random", users_mapped = False, evaluation = Evaluation.TRAIN_TEST_SPLIT, min_n_posrated = 20, min_n_negrated = 20, test_size = 0.2, random_state = 42,
                    max_users = 500)