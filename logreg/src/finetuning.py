...
def get_dataloaders(args_dict : dict, users_embeddings_ids_to_idxs : dict, papers_ids_to_categories_idxs : dict, papers_ids_to_categories_idxs_l2 : dict = None) -> tuple:
    train_ratings_dataloader = get_train_ratings_dataloader(args_dict, users_embeddings_ids_to_idxs, papers_ids_to_categories_idxs, papers_ids_to_categories_idxs_l2)
    assert len(train_ratings_dataloader) == args_dict["n_batches_total"]
    if args_dict["n_train_negative_samples"] > 0:
        train_negative_samples_dataloader = get_train_negative_samples_dataloader(args_dict["n_train_negative_samples"], papers_ids_to_categories_idxs, 
                                                                                  args_dict["n_batches_total"], args_dict["seed"], papers_ids_to_categories_idxs_l2)
...
def load_optimizer(finetuning_model : FinetuningModel, lr_transformer_model, l2_regularization_transformer_model, lr_other, l2_regularization_other,
    ...
    if finetuning_model.categories_embeddings_l2 is not None:
        param_groups.append({'params': finetuning_model.categories_embeddings_l2.parameters(), 'lr': lr_other, 'weight_decay': l2_regularization_other})
...
def process_batch(finetuning_model : FinetuningModel, args_dict : dict, optimizer : torch.optim.Optimizer, scheduler : torch.optim.lr_scheduler, train_ratings_batch : dict,
    ...
    with torch.autocast(device_type = 'cuda', dtype = torch.float16):
        ...
        train_ratings_scores, batch_negatives_scores = finetuning_model(eval_type = "train", input_ids_tensor = train_ratings_batch["input_ids"],
            ...
            category_idx_l2_tensor = train_ratings_batch.get("category_idx_l2", None))
        ...
        if train_negative_samples_batch is not None:
            train_negative_samples_scores = finetuning_model(eval_type = "negative_samples", input_ids_tensor = train_negative_samples_batch["input_ids"],
                                                            ...
                                                            category_idx_tensor = train_negative_samples_batch["category_idx"], category_idx_l2_tensor = train_negative_samples_batch.get("category_idx_l2", None))
...
if __name__ == "__main__":
    ...
    if args_dict["include_l2_categories"]:
        papers_ids_to_categories_idxs_l2 = load_papers_ids_to_categories_idxs_l2()
        n_categories_l2 = len(set(papers_ids_to_categories_idxs_l2.values()))
    else:
        papers_ids_to_categories_idxs_l2, n_categories_l2 = None, 0
    val_data = load_val_data(users_embeddings_ids_to_idxs, papers_ids_to_categories_idxs, papers_ids_to_categories_idxs_l2)
    train_ratings_dataloader, train_negative_samples_dataloader = get_dataloaders(args_dict, users_embeddings_ids_to_idxs, papers_ids_to_categories_idxs, papers_ids_to_categories_idxs_l2)
    finetuning_model = load_finetuning_model(args_dict["model_path"], device, val_data["val_users_embeddings_idxs"], n_unfreeze_layers = args_dict["n_unfreeze_layers"],
                                            ...
                                            pretrained_categories_embeddings = args_dict["pretrained_categories_embeddings"], n_categories_l2 = n_categories_l2)
