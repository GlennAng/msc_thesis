...
MODEL_CHOICES = ["gte-base-en-v1.5", "gte-large-en-v1.5", "specter2_base", "Qwen3-Embedding-0.6B", "Qwen3--Embedding-4B", "Qwen3-Embedding-8B"]
...
run_string = f"python {ProjectPaths.logreg_src_embeddings_path() / 'compute_embeddings.py'} --model_path {model_path} --embeddings_folder {embeddings_folder} --max_batch_size {batch_size}"
run_string += f" --max_sequence_length {MAX_SEQUENCE_LENGTH} {'--all_papers' if args_dict['all_papers'] else ''}"
os.system(run_string)
os.system(f"python {ProjectPaths.logreg_src_embeddings_path() / 'merge_embeddings.py'} {embeddings_folder}")