...
class FinetuningModel(nn.Module):
                    ...
                    with torch.autocast(device_type = self.device.type, dtype = torch.float16):
                        with torch.inference_mode():
                            results = self(eval_type = eval_type, input_ids_tensor = batch_input_ids_tensor, attention_mask_tensor = batch_attention_mask_tensor, 
                                           category_idx_tensor = batch_category_idx_tensor, user_idx_tensor = batch_user_idx_tensor).cpu()
    ...
    def compute_papers_embeddings(self, input_ids_tensor : torch.tensor, attention_mask_tensor : torch.tensor, category_idx_tensor : torch.tensor) -> torch.tensor:
        return self.perform_inference(self.eval_batch_size, input_ids_tensor, attention_mask_tensor, category_idx_tensor, eval_type = "test")

    def compute_val_ratings_scores(self, input_ids_tensor : torch.tensor, attention_mask_tensor : torch.tensor, category_idx_tensor : torch.tensor, user_idx_tensor : torch.tensor ) -> torch.tensor:
        return self.perform_inference(self.eval_batch_size, input_ids_tensor, attention_mask_tensor, category_idx_tensor, user_idx_tensor, eval_type = "val")

    def compute_val_negative_samples_scores(self, input_ids_tensor : torch.tensor, attention_mask_tensor : torch.tensor, category_idx_tensor : torch.tensor) -> torch.tensor:
        with torch.autocast(device_type = self.device.type, dtype = torch.float16):
            with torch.inference_mode():
...
def load_finetuning_model(finetuning_model_path : str, device : torch.device, val_users_embeddings_idxs : torch.tensor = None, n_unfreeze_layers : int = 4,
                          ...
                          projection_dim : int = 256, n_users : int = 4111, n_categories : int = 18, categories_embeddings_dim : int = 100) -> FinetuningModel:
    ...
    return FinetuningModel(transformer_model, projection, users_embeddings, categories_embeddings, val_users_embeddings_idxs, n_unfreeze_layers = n_unfreeze_layers)