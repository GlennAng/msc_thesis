        ...
        def wrapper(results: pd.DataFrame, *args, **kwargs):
            for column_name in column_names:
                if column_name not in results.columns:
                    raise ValueError(f"Column '{column_name}' not found in the DataFrame for '{method_name}'.")
        ...
        def wrapper(results: pd.DataFrame, *args, **kwargs):
            for column_name in column_names:
                if column_name in results.columns:
                    raise ValueError(f"Column '{column_name}' should not be present in the DataFrame for '{method_name}'.")
                return func(results, *args, **kwargs)
        return wrapper
    return decorator

throw_if_fold_idx_missing = lambda method_name: throw_if_missing(["fold_idx"], method_name)
throw_if_fold_idx_present = lambda method_name: throw_if_present(["fold_idx"], method_name)
throw_if_user_id_missing = lambda method_name: throw_if_missing(["user_id"], method_name)
...
def average_over_folds(results_before_avging_over_folds : pd.DataFrame) -> pd.DataFrame:
    ...
    results_after_avging_over_folds = results_before_avging_over_folds.groupby(group_columns).mean().reset_index()
...
def average_over_folds_with_std(results_before_avging_over_folds : pd.DataFrame) -> pd.DataFrame:
    ...
    results_means = results_means.rename(columns = {f"{score}": f"{score}_mean" for score in results_means.columns if score not in group_columns})
    ...
    results_stds = results_stds.rename(columns = {f"{score}": f"{score}_std" for score in results_stds.columns if score not in group_columns})
    results_after_avging_over_folds = pd.merge(results_means, results_stds, on = group_columns, how = "inner")
    return results_after_avging_over_folds
...
def average_over_users(results_after_avging_over_folds : pd.DataFrame) -> pd.DataFrame:
    ...
    non_group_columns = [column for column in results_after_avging_over_folds.columns if column not in group_columns]
    ...
    results_means = results_means.rename(columns = {f"{score}": f"{score}_mean" for score in non_group_columns})
    ...
    results_stds = results_stds.rename(columns = {f"{score}": f"{score}_std" for score in non_group_columns})
    results_after_avging_over_users = pd.merge(results_means, results_stds, on = group_columns, how = "inner")
...
def keep_only_n_most_extreme_users_for_all_hyperparameters_combinations_score(results_after_avging_over_folds : pd.DataFrame, score : Score, n_extreme_users : int, use_smallest : bool) -> tuple:
    ...
    n_most_extreme_users_train = results_after_avging_over_folds[["user_id", "combination_idx"] + [f"train_{score_name}"]]
    n_most_extreme_users_val = results_after_avging_over_folds[["user_id", "combination_idx"] + [f"val_{score_name}"]]
    extreme_func = pd.DataFrame.nsmallest if use_smallest else pd.DataFrame.nlargest
    n_most_extreme_users_train = n_most_extreme_users_train.groupby(["combination_idx"]).apply(lambda x: extreme_func(x, n_extreme_users, f"train_{score_name}")).reset_index(drop = True)
    n_most_extreme_users_val = n_most_extreme_users_val.groupby(["combination_idx"]).apply(lambda x : extreme_func(x, n_extreme_users, f"val_{score_name}")).reset_index(drop = True)
...
def average_over_n_most_extreme_users_for_all_hyperparameters_combinations_score(results_after_avging_over_folds : pd.DataFrame, score : Score, n_extreme_users : int, use_smallest : bool) -> pd.DataFrame:
    ...
    """
    n_most_extreme_users_train, n_most_extreme_users_val = keep_only_n_most_extreme_users_for_all_hyperparameters_combinations_score(results_after_avging_over_folds, score, n_extreme_users, use_smallest)
    results_after_avging_over_n_most_extreme_users_train = average_over_users(n_most_extreme_users_train)
    results_after_avging_over_n_most_extreme_users_val = average_over_users(n_most_extreme_users_val)
